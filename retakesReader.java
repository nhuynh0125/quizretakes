...
public class retakesReader {
	...
	public retakes read(String filename) throws IOException, ParserConfigurationException, SAXException {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				Integer month = Integer
						.parseInt(elem.getElementsByTagName("month").item(0).getChildNodes().item(0).getNodeValue());
				// day is integer 1..31
				Integer day = Integer
						.parseInt(elem.getElementsByTagName("day").item(0).getChildNodes().item(0).getNodeValue());
				// hour is integer 0..23
				Integer hour = Integer
						.parseInt(elem.getElementsByTagName("hour").item(0).getChildNodes().item(0).getNodeValue());
				// minute is integer 0..59
				Integer minute = Integer
						.parseInt(elem.getElementsByTagName("minute").item(0).getChildNodes().item(0).getNodeValue());
